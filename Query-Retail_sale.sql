...
-- Q.5 Write a SQL query to find the total number of transactions (transaction_id) made by each gender in each category. -- Q.7 Write a SQL query to calculate the average sale for each month. Find out best selling month in each year
SELECT 
...
SELECT 
    category, COUNT(DISTINCT customer_id) AS count_unique_cstrs
FROM
    retail_sales
GROUP BY category;
